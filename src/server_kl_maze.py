...
from util import visualize_reward, save, visualize_reward_gt, visualize_reward_twod, visualize_reward_twod_gt
...
def main(cfg: DictConfig):
    ...
    rollouts = load_rollouts(os.path.join(to_absolute_path('.'), "../jjh_data/expert_models/","twodmaze","final.pkl"))
    ...
    gt_c = []
    cnt_x = 0
    for x in np.linspace(110, 390, 40):
        cnt_x += 1
        cnt_y = 0
        
        for y in np.linspace(110, 390, 40):
            cnt_y += 1

            s, r, d, info = test_env.set_states(car=(x/20.0,y/20.0), goal=(250/20.0, 375/20.0), obs=(250/20.0, 250/20.0))
        ...
        if checkpoint_interval > 0 and round_num % checkpoint_interval == 0:
            ...
            for i in range(300):
                ...
                if render:
                    eval_env.render(mode='human')
                    time.sleep(0.005)
            # visualize_reward_twod(gail_trainer.gen_algo, lambda *args:  1.0 * gail_trainer.reward_test(*args) ,
            #                         state=states_l,
            #                       size=(20,20),
            #                       log_dir=log_dir,
            ...
            # visualize_reward_twod(gail_trainer.gen_algo, lambda *args: gail_trainer._running_norm(1.0 * gail_trainer.constraint_test(*args)) ,
            #                         state=states_l,
            #                       size=(cnt_x,cnt_y),
            #                       log_dir=log_dir,
            #                       round_num=int(round_num), 
            ...
            visualize_reward_twod(gail_trainer.gen_algo, lambda *args: -1*gail_trainer._running_norm( 1.0 * gail_trainer.constraint_test(*args) ),
                                    state=states_m,
                                  size=(cnt_x,cnt_y),
                                  log_dir=log_dir,
                                  round_num=int(round_num), 
