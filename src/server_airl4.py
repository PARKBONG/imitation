...
def main(cfg: DictConfig):
    ...
    import numpy as np
    def real(s, a, ns, d):
        xcost = 0.5 * np.abs(s[...,1] - s[...,0])
        xcost -= 2 * (np.abs(s[...,1] - s[...,0]) < 0.005)
        return xcost
    def cb(round_num):
        if checkpoint_interval > 0 and round_num % checkpoint_interval == 0:
            save(gail_trainer, os.path.join(log_dir, "checkpoints", f"{round_num:05d}"))
            visualize_reward(gail_trainer.gen_algo, gail_trainer.reward_train.predict_processed, env_id,log_dir,  int(round_num), "total", is_wandb, )
            visualize_reward(gail_trainer.gen_algo, lambda *args: gail_trainer.reward_train.predict_processed(*args) - real(*args),env_id,log_dir,  int(round_num), "constraint", is_wandb, )
