...
def main(cfg: DictConfig):
    ...
    rollouts = load_rollouts(os.path.join(to_absolute_path('.'), "../jjh_data/expert_models/","twodconst-v0","final.pkl"))
    ...
    name = 'ird' + comment
    wandb.init(project='test_bench-contact', sync_tensorboard=True, dir=log_dir, config=cfg, name=name)
    ...
    for x in np.linspace(-1.5, 1.5, 40):
        ...
        for y in np.linspace(-0.2, 0.8, 40):
            cnt_y += 1
